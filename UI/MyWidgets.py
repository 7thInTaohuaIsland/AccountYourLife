...
from SAK.GeneralTools import TimeTools
...
class TableWidgetClass(QWidget):
    def __init__(self, accountDictMonth, configDict, path, parent=None):
        ...
        self.values = {}
        self.path = path
        self.initUI(accountDictMonth, configDict, path)
        self.__itemChanged = False
        # 创建一个字典来存储表格值
        table_data_dict = {}
        # 遍历QTableWidget的所有单元格，并将值存储在字典中
        for row in range(self.tableWidget.rowCount()):
            for col in range(self.tableWidget.columnCount()):
                item = self.tableWidget.item(row, col)
                if item:  # 确保单元格不是空的
                    table_data_dict[(row, col)] = item.text()
                else:
                    table_data_dict[(row, col)] = None

    def initUI(self, accountDictMonth: dict, configDict: dict, path):
        self.tableWidget = QTableWidget(0, 6, self)  # 4 rows, 3 columns
        self.tableWidget.setHorizontalHeaderLabels(configDict["headers"])
        ...
        for key, accountDictDay in accountDictMonth.items():
            self.__addRow(currentRow, key, accountDictDay, configDict, path)
            currentRow += 1
    ...
    def __addRow(self, rowPosition, key, accountDict: dict, configDict: dict, path):
        ...
        try:
            ...
            if tempStr3 not in configDict["options"]:
    ...
    def insertNewEmptyRow(self, rowPosition):
        date = None
        accountDict = {"备注": "新建行"}
        self.__addRow(rowPosition, date, accountDict, self.configDict, self.path)
        self.__itemChanged = True
    ...
    def on_combo_box_changed(self, index):
        ...
        for key, combo in self.combo_boxes.items():
            if combo == sender:
                ...
                break
        # 显示消息框（可选）
        # QMessageBox.information(self, 'ComboBox Changed',
        #                         f'Option changed to {selected_text} in row {row}, column {col}')
    ...
    def on_date_changed(self, date):
        ...
        for key, dateEdit in self.date_edits.items():
            if dateEdit == sender:
                ...
                newDateStr = TimeTools().replace_date_if_not_match(dateStr, self.path[1], self.path[2])
                # 尝试将新字符串转换回 QDate 对象
...
class TreeWidgetClass(QWidget):
    ...
    def __init__(self, userDataDict, configDict, parent=None):
        super(TreeWidgetClass, self).__init__(parent)
        self.configDict = configDict
        self.initUI(userDataDict)

    def initUI(self, userDataDict: dict):
        ...
        for accountBookName, accountBook in userDataDict.items():
            ...
            for year, accountBookYear in accountBook.items():
                ...
                for month, accountBookMonth in accountBookYear.items():
                    ...
                    if not defaultSeted:
                        self.treeWidget.setCurrentItem(child1)
                        defaultSeted = True
    ...
    def set_current_item_by_path(self, tree_widget, path):
        """
        ...
        for top_level_index in range(tree_widget.topLevelItemCount()):
            top_level_item = tree_widget.topLevelItem(top_level_index)
