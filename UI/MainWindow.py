...
from SAK.FileIO import readDictFromJson, writeDictToJson
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        for accountName,accountYears in self.userData.items():
            for year, accountMonths in accountYears.items():
                ...
                self.userData[accountName][year] = dict(sorted(self.userData[accountName][year].items(), key=lambda item: self.sort_months(item[0])))
        # 将userData中的账本名、年、月加载到树中
        self.treeWidgetClass = TreeWidgetClass(self.userData,self.configDict)

        # 创建一个容器widget用于右边的splitter
        right_container = QWidget()
        self.right_layout = QVBoxLayout(right_container)
        self.table_widget_layout = QHBoxLayout()
        # 将userData中的符合memoryData的项加载到tabelWidget中
        try:
            ...
            for year, dataDictYear in accountBook.items():
                for month, dataDictMonth in dataDictYear.items():
                    if useDefaultPath:
                        ...
                        self.load_table(dataDictMonth,[accountName,year,month])
                        break
                    else:
                        if accountName == path[0] and year == path[1] and month == path[2]:
                            self.load_table(dataDictMonth, [accountName, year, month])
                            break
        self.treeWidgetClass.set_current_item_by_path(self.treeWidgetClass.treeWidget,path)
        ...
        self.treeWidgetClass.deleteItemSignal.connect(self.on_tree_item_deleted)
        self.treeWidgetClass.addItemSignal.connect(self.on_tree_item_add)
        # 获取主屏幕的几何信息
    ...
    def on_tree_item_double_clicked(self, item, column):
        ...
        if len(path)==3:
            ...
            if self.tableWidgetClass.isAnyItemChanged():
                self.ask_to_save()
            # 根据新的数据创建新的表格控件
            dataDictMonth = self.userData[path[0]][path[1]][path[2]]
            self.load_table(dataDictMonth,path)
    ...
    def load_table(self, dataDictMonth,path):
        if self.current_table_widget:
            self.right_layout.removeWidget(self.current_table_widget)
            self.current_table_widget.deleteLater()
        self.current_table_widget = TableWidgetClass(dataDictMonth, self.configDict,path, self)
        self.tableWidgetClass = self.current_table_widget
        self.table_widget_layout.addWidget(self.current_table_widget)
    ...
    def on_tree_item_deleted(self):
        ...
        while treeItem:
            path.insert(0, treeItem.text(0))  # 在路径列表的开头插入当前项的文本
            treeItem = treeItem.parent()
        self.load_table(self.userData[path[0]][path[1]]["1月"],path = [path[0],path[1],"1月"])
